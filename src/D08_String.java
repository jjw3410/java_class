...
public class D08_String {
	...
	public static void main(String[] args) {
		...
		//replace:일치하는 문자열을 모두 원하는 문자열로 교체한 결화를 반환한다
		str="abc123@naver.com";
		str=str.replace("@naver.com", "@gmail.com");//교체후 str에 다시 넣어준다
		...
		//contains:문자열에 해당 문자열이 포함되어 있는지를 검사한다
		...
		System.out.println("image로 시작하나요 ?"+str.startsWith("image"));
		...
		//substring:문자열의 원하는 부분을 문자열로 반환한다
		...
		//valueOf:다른 타입의 값을 문자열로 변환한다(해당 객체의 toString결과를 반환받는다)
		...
		//자를때 사용했던 문자열이 필요하다면 세번째 파라미터를 true로 활성화
		StringTokenizer st=new StringTokenizer(str,"/",true);//true를 넣으면 / 잘라서 같이 나온다  나오기 싫으면 true를 배기
