...
public class B14_Array2 {
	...
	public static void main(String[] args) { 
		...
		//System.out.println(arr2[1].length);// null의length는 없으므로 에러
		System.out.println("arr2에 저장된 배열중 2번째 배열의 길이"+arr2[2].length);
		System.out.println("arr2에 저장된 배열중 3번째 배열의 길이"+arr2[3].length);
		//길이를 활용하여 n차원 배열은 n중 반복문으로 모두 탐색할수 있다.
		for(int i=0;i<arr2.length;++i) {
			int[]arr=arr2[i];
			
			if(arr!=null) {
				for(int j=0;j<arr.length;++j) {
					System.out.print(arr2[i][j]+" ");//System.out.println(arr[j]); 와 같음
